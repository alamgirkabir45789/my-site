...
import ModuleAddForm from '../form/ModuleAddForm';
import ModuleEditForm from '../form/ModuleEditForm';
import { deleteRangeModule, getModuleByQuery, handleOpenModuleSidebar } from '../store/actions';
import ModuleExpandRow from './ModuleExpandRow';
import { ModuleTableColumns } from './ModuleTableColumn';
...
const ModuleList = () => {
  ...
  // ** Global States
  const { modules, total, selectedModule, openModuleSidebar, queryData } = useSelector(
    ({ modules }) => modules
  );
  // ** Size States
  ...
  const toggleSidebar = () => store.dispatch(handleOpenModuleSidebar(!openModuleSidebar));
  ...
  const dataToRender = () => {
    ...
    if (queryData.length > 0) {
      ...
      return modules.slice(0, rowsPerPage);
    }
  };
  ...
  // ** Delete Rang
  const handleDeleteModuleRange = () => {
    dispatch(deleteRangeModule(selectedRowId));
    setSelectedRowId([]);
    handleClearSelected();
  };
  ...
  return (
    <div>
      ...
      <Card>
        <div>
          <CardHeader>
            <CardTitle tag="h2">Module</CardTitle>
          </CardHeader>
          <TableCustomerHeader
          ...
          <DataTable
            ...
            paginationServer
            expandableRows
            expandableRowsComponent={<ModuleExpandRow data={data => data} />}
            expandOnRowClicked
            persistTableHead
      ...
      {selectedModule !== null && openModuleSidebar ? (
        <ModuleEditForm
          data={selectedModule}
          open={openModuleSidebar}
          toggleSidebar={toggleSidebar}
        />
      ) : openModuleSidebar ? (
        <ModuleAddForm open={openModuleSidebar} toggleSidebar={toggleSidebar} />
      ) : null}
    </div>
  );
};
