...
const IncompleteTypeList = () => {
  ...
  const { items, isOpenSidebar, total, selectedItem } = useSelector(
    ({ incompleteTypeReducer }) => incompleteTypeReducer
  );
  ...
  //#region Effects
  useEffect(() => {
    dispatch(fetchIncompleteTypesByQuery({ page: currentPage, rowsPerPage }));
  ...
  const handleFilter = val => {
    setSearchTerm(val);
    dispatch(fetchIncompleteTypesByQuery({ q: val, rowsPerPage, page: currentPage }));
  };

  const handleSort = (column, direction) => {
    const { selector } = column;
    dispatch(fetchIncompleteTypesByQuery({ sortedBy: direction, sortedColumn: selector }));
  };
